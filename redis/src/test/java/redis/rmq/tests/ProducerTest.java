...
public class ProducerTest extends Assert {
    ...
    public void raceConditionsWhenPublishing() throws InterruptedException {
        Producer slow = new SlowProducer(RedisUtil.getRedisClient(), "foo");
        Consumer c = new Consumer(RedisUtil.getRedisClient(), "a subscriber", "foo");

        slow.publish("a");
        Thread t = new Thread(new Runnable() {
            public void run() {
                Producer fast = new Producer(RedisUtil.getRedisClient(), "foo");
                fast.publish("b");
            }
        });
        t.start();
    ...
    public void eraseOldMessages() {
        ...
        Consumer nc = new Consumer(RedisUtil.getRedisClient(), "new subscriber",
                "foo");
    ...
    public void expiredMessages() throws InterruptedException {
        Consumer c = new SlowConsumer(RedisUtil.getRedisClient(), "a consumer",
                "foo", 2000L);
        Producer p = new Producer(RedisUtil.getRedisClient(), "foo");
